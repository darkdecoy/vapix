...
control1.schedule.set_schedule(name=schedule_name, token=token)

for x in range(1):
    control1.schedule.schedules[token].add_event(name=name, start=start, end=end)

control1.doorcontrol.doors['Door 1'].set_unlockschedules(token=list(control1.schedule.schedules[token].children), reset=False)
